...
configure_dotfiles() {
  ...
  if [ -d "$config_dir" ]; then
    log "📦 Stowing .config directory..."
    cd "$dotfiles_dir" || error_exit "Failed to change directory to dotfiles"
    stow .config || log_warning "Failed to stow .config directory."
  else
    log_warning ".config directory not found in dotfiles."
  fi

  # Reload Alacritty configuration (assuming Alacritty config is in .config)
